...
class KusteAIAgent:
    def __init__(self) -> None:
        ...
        self.guard = CommandGuard(owner_pin=self.settings.owner_pin)
        self.adb = ADBBridge(guard=self.guard, adb_path=self.settings.adb_path)
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "Если пользователь просит опасные действия (удаление, форматирование, uninstall), "
            "напоминай, что нужна явная авторизация владельца.\n\n"
            f"Память:\n{memories_text}"
        )
    ...
    def approve_owner(self, pin: str) -> bool:
        return self.guard.approve(pin)
    ...
    def adb_shell(self, command: str) -> str:
        return self.adb.run_shell(command)
